...
int tpd_get_pinctrl(struct i2c_client *client)
{
        ...
        ILI_INFO("[tpd] mt_tpd_pinctrl+\n");
        pinctrl1 = devm_pinctrl_get(client->adapter->dev.parent);
        if (IS_ERR(pinctrl1)) {
                ret = PTR_ERR(pinctrl1);
                ILI_ERR("Cannot find pinctrl1!\n");
                return ret;
        }
        pins_default = pinctrl_lookup_state(pinctrl1, "default");
        if (IS_ERR(pins_default)) {
                ret = PTR_ERR(pins_default);
                ILI_INFO("Cannot find pinctrl default %d!\n", ret);
        }
        ...
        if (IS_ERR(pin_i2c_mode)) {
                ...
                ILI_ERR("Failed to get pinctrl state:%s, ret:%d",
                                "ts_i2c_mode", ret);
        }
	...
	if (IS_ERR(rst_output0)) {
		...
		ILI_ERR("Cannot find pinctrl ts_reset_suspend!\n");
		return ret;
	}
	rst_output1 =
	...
	if (IS_ERR(rst_output1)) {
		...
		ILI_ERR("Cannot find pinctrl ts_reset_active!\n");
		return ret;
	}
	ILI_INFO("[tpd] mt_tpd_pinctrl-\n");
        return 0;
...
static int ilitek_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	err = pinctrl_select_state(pinctrl1, pin_i2c_mode);
        if (err < 0)
                ILI_ERR("Failed to select default pinstate, err:%d", err);
...
int ili_interface_dev_init(struct ilitek_hwif_info *hwif)
{
	...
	if (ret != 0) {
                ILI_ERR("unable to add i2c driver.");
        }
	return ret;
}
