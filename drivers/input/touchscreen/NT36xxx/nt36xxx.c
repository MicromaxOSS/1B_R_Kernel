...
void nt36xxx_tpd_gpio_output(int pin, int level);
void nt36xxx_tpd_gpio_as_int(int pin);
...
static const struct nvt_ts_trim_id_table trim_id_table[] = {
	...
	{.id = {0xFF, 0xFF, 0xFF, 0x72, 0x66, 0x03}, .mask = {0, 0, 0, 1, 1, 1},
		.mmap = &NT36772_memory_map, .carrier_system = 0},
	{.id = {0x0B, 0x00, 0x00, 0x25, 0x65, 0x03}, .mask = {1, 0, 0, 1, 1, 1},
...
void nt36xxx_tp_reset(void) {
        nt36xxx_tpd_gpio_output(ts->reset_gpio, 0);
        mdelay(5);
        nt36xxx_tpd_gpio_output(ts->reset_gpio, 1);
}
...
void nvt_reset_pin_ctl(int onoff)
{
  NVT_LOG("nvt tp reset onoff:%d",onoff);
  if(onoff)
  	nt36xxx_tpd_gpio_output(ts->reset_gpio, 1);
  else
  	nt36xxx_tpd_gpio_output(ts->reset_gpio, 0);
...
info_retry:
	...
	//printk("wxs nvt_get_fw_info %d,%d,%d,\n",fw_ver,ts->abs_x_max,ts->max_button_num);
	if ((buf[1] + buf[2]) != 0xFF) {
...
static int nvt_irq_registration(void) {
	...
	if (node) {
		...
		nt36xxx_tpd_gpio_as_int(ts->irq_gpio);
	...
	for (retry = 5; retry > 0; retry--) {
		...
		for (list = 0;
			...
			for (i = 0; i < NVT_ID_BYTE_MAX; i++) {
				//NVT_LOG("i=%d mask=%d, buf=0x%02X, id=0x%02X\n",i, trim_id_table[list].mask[i], 
...
extern void NvtGet_FW_Ver(uint8_t *buffer);
...
static ssize_t read_tp_info(struct file *file, char __user *buffer, size_t count, loff_t *ppos) {
	...
	strcpy(tp_proc_buffer, "NT36525B");
	...
	ptr = page;
	ptr += sprintf(page, "%s:%s:0x%02x\n", tp_module_buffer,tp_proc_buffer,fw_ver);
...
static int creat_proc_tp_info(void) {
...
static int tpd_get_pinctrl(struct i2c_client *client)
{
        int ret;

        NVT_ERR("[tpd] mt_tpd_pinctrl+\n");
        pinctrl1 = devm_pinctrl_get(client->adapter->dev.parent);
        if (IS_ERR(pinctrl1)) {
                ret = PTR_ERR(pinctrl1);
                NVT_ERR("Cannot find pinctrl1!\n");
                return ret;
        }
        pins_default = pinctrl_lookup_state(pinctrl1, "default");
        if (IS_ERR(pins_default)) {
                ret = PTR_ERR(pins_default);
                NVT_ERR("Cannot find pinctrl default %d!\n", ret);
        }
        ...
        if (IS_ERR(pin_i2c_mode)) {
                ...
                NVT_ERR("Failed to get pinctrl state:%s, ret:%d",
                                "ts_i2c_mode", ret);
        }
        ...
        if (IS_ERR(rst_output0)) {
                ...
                NVT_ERR("Cannot find pinctrl ts_reset_suspend!\n");
                return ret;
        }
        rst_output1 =
        ...
        if (IS_ERR(rst_output1)) {
                ...
                NVT_ERR("Cannot find pinctrl ts_reset_active!\n");
                return ret;
        }
        NVT_ERR("[tpd] mt_tpd_pinctrl-\n");
        return 0;
...
static int32_t nvt_ts_probe(struct i2c_client *client,
	...
	ret = pinctrl_select_state(pinctrl1, pin_i2c_mode);
	if (ret < 0)
		NVT_ERR("Failed to select default pinstate, err:%d", ret);
