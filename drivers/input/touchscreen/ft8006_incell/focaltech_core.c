...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "ft,ft8006"
...
#endif
void ft8006_tpd_gpio_output(int pin, int level);
void ft8006_tpd_gpio_as_int(int pin);
...
void fts_reset_pin_ctl(int onoff)
{
  ...
  if(onoff)
  	ft8006_tpd_gpio_output(fts_data->pdata->reset_gpio, 1);
	else
  	ft8006_tpd_gpio_output(fts_data->pdata->reset_gpio, 0);
}
...
{
/*Commenting to resolve compilation error*/
#if 0
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    ft8006_tpd_gpio_as_int(ts_data->pdata->irq_gpio);
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            ...
            ft8006_tpd_gpio_output(ts_data->pdata->reset_gpio, 0);
            msleep(1);
...
{
    ft8006_tpd_gpio_output(ts_data->pdata->reset_gpio, 1);
    return 0;
}
#endif
static void fts_platform_data_init(struct fts_ts_data *ts_data)
{
    ...
    pdata->reset_gpio = 0;
    pdata->x_min = 0;
    pdata->x_max = tpd_dts_data.tpd_resolution[0];
    pdata->y_min = 0;
    pdata->y_max = tpd_dts_data.tpd_resolution[1];
...
static int creat_proc_tp_info(void)
{
    ...
    if (tp_info_entry == NULL){
        printk("create touch_panel_info fail\n");
    }
...
static int tpd_get_pinctrl(struct i2c_client *client)
{
        ...
        if (IS_ERR(pins_default)) {
                ...
                FTS_ERROR("Cannot find pinctrl default %d!\n", ret);
        }
        ...
        if (IS_ERR(eint_output1)) {
                ...
                FTS_ERROR("Cannot find pinctrl ts_eint_high!\n");
                return ret;
        }
	rst_output0 =
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret < 0)
           FTS_ERROR("Failed to select default pinstate, err:%d", ret);
...
void ft8006_driver_exit(void)
{
    FTS_FUNC_ENTER();
    tpd_driver_remove(&tpd_device_driver);
    FTS_FUNC_EXIT();
}
/*Specific driver init calling from tpd control*/
#if 0
