...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FPSENSOR_IOC_INIT:
        ...
        wake_lock_init(fpsensor_dev->device, &fpsensor_timeout_wakelock, WAKE_LOCK_SUSPEND, "fpsensor timeout wakelock");
...
static int __init fpsensor_init(void)
{
    ...
    FUNC_ENTRY();
	pr_err("###Justin %s : %d Start ", __func__, __LINE__);
	pr_info("wxs fpsensor_init \n");
#if FPSENSOR_SPI_BUS_DYNAMIC
