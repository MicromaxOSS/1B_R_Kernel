...
static irqreturn_t fpsensor_irq(int irq, void *handle)
{
    ...
    ** since this is interrupt context (other thread...) */
    fpsensor_debug(INFO_LOG, " fpsensor_irq fired!\n");
    smp_rmb();
...
#ifdef FPSENSOR_PROC_INFO
static int fpsensor_info_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n", "HEDAYUAN:ICNF7332AL");
	return 0;
};

static int fpsensor_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, fpsensor_info_read, NULL);
};

static const struct file_operations fpsensor_proc_info_fops = {
        .owner = THIS_MODULE,
        .read = seq_read,
        .open = fpsensor_info_open,
};
...
#endif
{
    ...
    wake_lock_init(&fpsensor_dev->spi->dev,&g_fpsensor->ttw_wl, WAKE_LOCK_SUSPEND, "fpsensor_ttw_wl");
    fpsensor_dev->device_available = 1;
#if FP_NOTIFY
...
#ifdef FPSENSOR_PROC_INFO
	proc_create("fpsensor_info", 0664, NULL, &fpsensor_proc_info_fops);
#endif
    return status;
}
