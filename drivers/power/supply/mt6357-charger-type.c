...
void do_charger_detect(struct mtk_charger_type *info, bool en)
{
	...
	pr_notice("%s connected = %d type:%d usb_type:%d\n", __func__, (int)en, prop2.intval, prop3.intval);
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (info->type == POWER_SUPPLY_TYPE_USB)
			val->intval = 500000;
		break;
	...
	case POWER_SUPPLY_PROP_STATUS:
		if(info->chg_dev == NULL) {
			pr_notice("%s : primary_chg not yet initialized \n", __func__);
		...
		if (info->type == POWER_SUPPLY_USB_TYPE_UNKNOWN) {
			...
			if(info->chg_dev)
				charger_dev_is_charging_done(info->chg_dev,&done);
			if(done)
...
static int mt6357_charger_type_probe(struct platform_device *pdev)
{
	...
	if (info->bc12_active) {
		...
		ret = devm_request_threaded_irq(&pdev->dev,
			...
			chrdet_int_handler, IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING, "chrdet", info);
		if (ret < 0)
