...
static int psc5415e_do_event(struct charger_device *chg_dev, unsigned int event, unsigned int args)
{
	...
	if(event == EVENT_FULL || event == EVENT_RECHARGE)
	{
                psy = power_supply_get_by_name("mtk_charger_type");
                if (!psy) {
                        pr_err("%s %d: cannot get power supply type\n",__func__,__LINE__);
                }
                else
                {
                        pr_err("%s %d: SUCCESS power supply type\n",__func__,__LINE__);
...
static int psc5415e_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	psc5415e_reg_config_interface(0x05, 0x82);
	psc5415e_reg_config_interface(0x04, 0x14);	/* 100mA */
