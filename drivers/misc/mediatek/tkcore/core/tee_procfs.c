						...
						unsigned int shm_len)
{
	...
	pr_err("###Justin tkcoredrv: shm length: %u  BUF_SIZE %d rb_len %u diff %d\n",
				shm_len, TEE_LOG_CTL_BUF_SIZE, rb_len, (shm_len - TEE_LOG_CTL_BUF_SIZE));
...
static int init_klog(struct klog *klog, struct tee *tee)
{
	...
	if (init_klog_shm_args(tee, &shm_pa,
						&shm_len) < 0) {
		pr_err("###Justin init_klog_shm_args failure \n");
		return -1;
	}
	...
	if (init_klog_shm(klog,
					shm_pa,
					shm_len) < 0) {
		pr_err("###Justin init_klog_shm failure \n");
		return -1;
	}
	...
	pr_err("###Justin init_klog creating thread \n");
...
int tee_init_procfs(struct tee *tee)
{
	...
	rc = init_klog(&klog, tee);
	if (rc < 0) {
		pr_err("###Justin init_klog failure \n");
