...
char camera_info[camera_info_size] = { 0 };
...
static int camera_info_read(struct seq_file *m, void *v)
{
	pr_debug("%s %s\n", __func__, camera_info);
	seq_printf(m, "%s\n", camera_info);
	return 0;
};

static int camera_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, camera_info_read, NULL);
};
...
enum IMGSENSOR_RETURN imgsensor_proc_init(void)
{
	...
	proc_create("cam_info", 0664, NULL, &fcamera_proc_fops_get_camera_info);
