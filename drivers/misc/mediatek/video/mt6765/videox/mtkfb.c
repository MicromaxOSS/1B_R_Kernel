...
static int fix_lcd_proc_info(void)
{
...
#endif
	char *lcd_name[5] = {"ili9882h_720x1600_incell", "ft8006_720x1600_incell", "ft8006_720x1600_incell_pd6511", "nt36525b_720x1600_incell", "nt36525b_720x1600_sc_incell"};
	...
	if (!strcmp(mtkfb_lcm_name, lcd_name[0])) {
		...
		strcpy(lcd_glass_buffer, "PANDA");
	} else if (!strcmp(mtkfb_lcm_name, lcd_name[2])) {
		strcpy(lcd_proc_buffer, "ZHONGGUANGDIAN");
		strcpy(lcd_module_buffer, "FT8006S");
		strcpy(lcd_glass_buffer, "TRULY");
	} else if (!strcmp(mtkfb_lcm_name, lcd_name[3])) {
		strcpy(lcd_proc_buffer, "HUAXIAN");
		strcpy(lcd_module_buffer, "NT36525B");
		strcpy(lcd_glass_buffer, "SHARP");
	} else if (!strcmp(mtkfb_lcm_name, lcd_name[4])) {
		strcpy(lcd_proc_buffer, "HUAXIAN");
		strcpy(lcd_module_buffer, "NT36525B");
		strcpy(lcd_glass_buffer, "CTC");
	} else {
...
static int lcd_info_read(struct seq_file *m, void *v)
{
	fix_lcd_proc_info();
...
{
	return single_open(file, lcd_info_read, NULL);
};
...
#ifdef LCD_PROC_INFO
	proc_create("lcd_info", 0664, NULL, &lcd_proc_info_fops);
#endif
	MSG_FUNC_LEAVE();
...
#if defined(CONFIG_MMX_ILI9882_NEED_SHUTDOWN_BEFORE_POWEROFF)
extern void ili9882_shutdown_power(void);
#endif

#if defined(CONFIG_MMX_FT8006_NEED_SHUTDOWN_BEFORE_POWEROFF)
extern void ft8006_shutdown_power(void);
#endif

#if defined(CONFIG_MMX_NT36525B_NEED_SHUTDOWN_BEFORE_POWEROFF)
extern void nt36525b_shutdown_power(void);
extern void nt36525b_sc_shutdown_power(void);
extern int nt36525b_is_sc_module(void);
#endif
...
static void mtkfb_shutdown(struct platform_device *pdev)
{
	...
	#if defined(CONFIG_MMX_NT36525B_NEED_SHUTDOWN_BEFORE_POWEROFF)
	if(nt36525b_is_sc_module())
		nt36525b_sc_shutdown_power();
	else
		nt36525b_shutdown_power();
	#endif
