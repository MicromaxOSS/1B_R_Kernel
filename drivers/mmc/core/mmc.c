...
static struct mem_type memtype[] = {
...
static ssize_t read_mem_info(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int len = 0;
...
static int create_proc_mem_info(void)
{
	struct proc_dir_entry *mem_info_entry = NULL;

	mem_info_entry = proc_create("mem_info", 0444, NULL, &mem_proc_info_fops);
	if (mem_info_entry == NULL){
		printk("create mem_info fail\n");
	}
	return 0;
...
static int create_proc_cpu_info(void)
{
	struct proc_dir_entry *cpu_info_entry = NULL;

	cpu_info_entry = proc_create("cpu_info", 0444, NULL, &proc_cpu_info_fops);
	if (cpu_info_entry == NULL) {
		printk("create cpu_info fail\n");
	}
	return 0;
}
#endif
...
#ifdef MEM_PROC_INFO
	create_proc_mem_info();
	if (0 != card->raw_cid[0]){
		u32 i,count;
		char buf[MEM_PROC_BUFFER_MAX_SIZE]={0};
		count = sizeof(memtype)/sizeof(struct mem_type);
		for(i = 0 ; i < count ; i++ ){
			if(strncmp(memtype[i].prod_name,card->cid.prod_name,5) == 0)
				 break;
			}
			if(i == count)
				 sprintf(buf,"unknown!");
